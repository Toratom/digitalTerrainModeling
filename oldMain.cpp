...
//Grid info
unsigned int g_gridWidth = 0; //Nb de colonnes de la grille de discretisation (image)
unsigned int g_gridHeight = 0; //Nb de lignes de la grille de discretisation (image)
unsigned int g_nbOfLayers = 0;
float g_cellWidth = 0;
float g_cellHeight = 0;
...
// GPU objects - Buffers
//GLuint g_colVbo = 0;
//GLuint g_terrainNormalVbo = 0; //Normale au sol
GLuint g_gridPosVbo = 0;
GLuint g_terrainPropsVbo = 0; //b0, b1, ..., bNbOfLayers
...
void Mesh::init() {
    //A appeler une seule fois lors de l'init sinon appeler update
    //Update cpu
    ...
    glBufferStorage(GL_ARRAY_BUFFER, vertexBufferSize, m_vertexPositions.data(), GL_DYNAMIC_STORAGE_BIT); //Pas trop d'idee pour le flag, il faudrait plutot 0 car l'utilisateur ne modifie pas donne
    ...
    glBufferStorage(GL_ELEMENT_ARRAY_BUFFER, indexBufferSize, m_triangleIndices.data(), 0);
...
void initGPUprogram() {
    ...
    CheckGlErrors("Vertex and Fragment Shader");
